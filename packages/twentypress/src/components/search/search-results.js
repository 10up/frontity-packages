		...
		<>
			<ArchiveHeader label="Search" labelColor={primary}>
				...
				<IntroText size="thin">
					{isEmpty ? (
						<Text>
							We could not find any results for your search. You can give it another
							try through the search form below.
						</Text>
					) : (
						<Text>
							We found {total} {total === 1 ? 'result' : 'results'} for your search.
						</Text>
					)}
				</IntroText>
			</ArchiveHeader>
			...
			{isEmpty ? (
				...
				<Archive showExcerpt showMedia={false} />
			)}
		</>
	);
};
